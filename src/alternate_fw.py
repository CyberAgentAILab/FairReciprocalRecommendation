...
def nsw_maximize(
    ...
    """Return stochastic policy for left and right computed via NSW maximization.
...
def sw_maximize(
    ...
    """Return stochastic policy for left and right computed via SW maximization.
...
def alternate_fw(
    ...
    """Return stochastic policy for left and right computed via alternate Frank-Wolfe algorithm.
...
def __fw_one_step(
    ...
    constraints = [b[:, num_left * m : num_left * (m + 1)] @ ones_num_left == 1 for m in range(num_left)] + [b[:, m : num_left**2 : num_left] @ ones_num_left == 1 for m in range(num_left)] + [b >= 0]
